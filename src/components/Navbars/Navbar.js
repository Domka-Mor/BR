		          		...
		          		<nav className="container-fluid nav-min fixed-top">
			                ...
			                <div className='pt-2'>	
			                	<ul className={this.state.isOpen?
						            ...
						            {token && (                  
						                <Link2 to='/'>
						                   <button className='nav-text' onClick={() => {logout(); this.handleToggle();}}>Logout</button>
						                </Link2>
						            )}
						            {!token && (                  
						                <Link2 to="/auth">
						                   <button className='nav-text' onClick={() => {login(); this.handleToggle();}}>Login</button>
						                </Link2>
						            )}
			                	</ul>
			            	</div>
		       		...
		       		<div className='d-none d-lg-block py-3'>
						<nav className="navbar navbar-expand-lg fixed-top">									
							...
							<span className="navbar-text">
					            ...
					            {token && (                  
					                <Link2 to='/'>
					                   <button className="nav-text" onClick={() => {logout();}}>Logout</button>
					                </Link2>
					            )}	
					            {!token && (
					            <Link2 to="/auth">
					                   <button className="nav-text" onClick={() => {login();}}>Login</button>
					            </Link2> 
					            )}	 
							</span>
						</nav>
